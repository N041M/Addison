...
class ServerContext:
    ...
    if TYPE_CHECKING:
        ...
        # Tri-state (secret_presence.SecretPresence: present | absent | unknown) —
        # the send path branches on all three. THE ONE PRESENCE READ THAT STILL
        # TOUCHES THE OS, because it is the one with a person behind it; every other
        # consumer reads the recorded answer (Store.secret_presence, plan §4.1).
