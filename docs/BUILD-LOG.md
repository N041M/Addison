...
hiding* to *artifact disabling*); `CLAUDE.md`, `architecture.md`, `data-model.md`,
...
0. **"Which data directory?" needs exactly one owner.** The mistake below was
   ...
   `denylisted_roots` and `call_is_forbidden` now **require** a data dir, with no
...
half of it comment and docstring; that ratio is intentional here) holds
...
(verdict: amend-then-build; six MUST-FIX findings integrated, and the review caught
...
- **`guards.set` is the G4 anchor caller** (rpc/guards.py): validate, compute
  ...
  if the anchor cannot mint), then persist. `mint_anchor` gained fingerprint
...
The lesson is the standing one: the test gap was structural, since the only
...
one path that legitimately skips the notice (sidecar cold-start, where the
...
REDESIGN, because the drafted quality-first silently overrode the user's standing
...
- Both flows are **propose/confirm RPCs whose fields are core-derived or canned**:
  ...
  constants.
- `costPlan.apply`: validate, skip if already in effect, then **snapshot, REFUSING
  ...
  the asymmetry is noted in both places), then **one atomic `Store` commit** so a
...
falls forward exactly as before, and that case is tested too, so the rule cannot
