...
| [`context-budget-plan.md`](context-budget-plan.md) | **BUILT 2026-08-14.** Owns the Context Budget Manager: the two pure modules (`context_budget.py`, `context_continuation.py`), the measurement beside `on_usage`, the turn-boundary check in `rpc/conversation.py`, the summary call through `model_router.resolve()`, the continuation with lineage, the note, and the over-window sentence spec §4.8 had claimed for months and nothing had ever produced. States every failure path (all of which do nothing, silently), the judgement that the Setup Assistant relay is refused, and three honest limits ([`KNOWN-GAPS.md`](KNOWN-GAPS.md) tracks those). Spec §4.8 remains the design and owns the five hard rules. |
