...
| [`step-7-mcp-plan.md`](step-7-mcp-plan.md) | **DONE FOR v1** — phases 1–4 of five built (2026-08-06 configuration, then three on 2026-08-07: discovery, dispatch through the ordinary gate as HIGH and destructive, and output handling — what a server sends back is redacted, bounded by one shared budget, and what Addison won't carry is named rather than dropped). Phase 5 (stdio under containment; SAFE admission) is a recorded later option. Build order for the MCP *client*, unblocked by the dev-only-for-v1 decision. Both §5 decisions are answered (HTTP-only transport; a section per server on the Tools surface); the phase entries carry ten more taken while building. |
