...
| [`step-8-automation-plan.md`](step-8-automation-plan.md) | **PHASE 1 OF 4 BUILT** (2026-08-07, the day it was written — the fence closing the open arming path, and the inert `automations` table). Build order for OS-run automation and the per-automation-nonce keyword gate: four phases (the fence + the table, authoring, the gate + arming, state honesty + Simple). The nonce decision is the owner's (2026-08-07); the plan's §5 settles the engineering decisions around it, and the phase-1 entry carries six more taken while building. |
