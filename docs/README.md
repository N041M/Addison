...
| [`messaging-channel-plan.md`](messaging-channel-plan.md) | **PROPOSED 2026-08-22, not scheduled.** Controlling Addison from a phone: outbound-only transport with no listener in any phase, a plural `ChannelAdapter` protocol with Telegram's Bot API first, pairing as the authorization boundary (the `automation_nonce` code shown on the desktop and sent from the phone; unknown senders ignored in silence), and a **remote floor** that is a closed set of tool ids asserted to be a subset of `visible_tools(SAFE)`. Owns the full API surface (modules, RPC methods, the two tables and their capture decisions, the keychain namespace) and the four-phase build order, plus eleven owner decisions and the limits that survive success. The deferral in [`../CLAUDE.md`](../CLAUDE.md) is unchanged; design-doc §7.10 is its origin and stays as history. |
