...
| [`phase-3-review-surface-plan.md`](phase-3-review-surface-plan.md) | Approved 2026-07-25, not started. Blocked on step 8 (6 landed 2026-08-06, 7 on 2026-08-07). |
| [`step-8-automation-plan.md`](step-8-automation-plan.md) | **PHASES 1–2 OF 4 BUILT** (2026-08-07, the day it was written — the fence closing the open arming path, the inert `automations` table, and authoring: `create_automation` with its four-refusal door, chat-only plist preview, and the Settings drafts section). Build order for OS-run automation and the per-automation-nonce keyword gate: four phases (the fence + the table, authoring, the gate + arming, state honesty + Simple). The nonce decision is the owner's (2026-08-07); the plan's §5 settles the engineering decisions around it, and the phase entries carry eleven more taken while building. |
