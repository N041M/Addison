...
| [`input-attachments-plan.md`](input-attachments-plan.md) | **PROPOSED 2026-08-23, owner decisions open.** Image and document INPUT: attach/drag a file into chat and have the model actually see it. Owns the four verified facts about the half-built `read_file` path (unreachable, stub extraction, no image-block path, real guards), the design (attachments as user-message parts; bytes in the shell, parsing in the core; send-time vision gate; screening and persistence decisions), the three phases, and the five owner decisions. The [`KNOWN-GAPS.md`](KNOWN-GAPS.md) "No file-attach/drop UI" entry now points here. |
