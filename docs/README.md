...
| [`phase-3-review-surface-plan.md`](phase-3-review-surface-plan.md) | Approved 2026-07-25, **unblocked, and its docs-first phase is DONE (2026-08-08)** — no surface code is written. Steps 6–8 landed 2026-08-06/07 and the plan's own three prerequisites closed 2026-08-08, each in its own change; the Phase-3 redefinition is now in every document that defines the phase, with a claims row holding the line. What is left is the build, starting at its §1. |
| [`step-8-automation-plan.md`](step-8-automation-plan.md) | **COMPLETE — all four phases built 2026-08-07**, the day it was written: the fence closing the open arming path, the inert `automations` table, authoring (`create_automation`), the keyword gate and arming (the per-automation nonce, plus the shell surface that owns `~/Library/LaunchAgents` and builds the plist itself), and state honesty + Simple's disabled rows. The nonce decision is the owner's; §5 settles the engineering decisions around it, the phase entries carry fifteen more taken while building, and §7 is the record of every document line that changed when arming became real. |
