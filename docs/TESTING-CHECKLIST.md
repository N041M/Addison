...
**Expect:** the window paints in the right theme immediately, with no white flash,
...
left is the **lowercase-`a` mark** alone (no "Addison" wordmark beside it), and it
...
starfield, with **no seeded "welcome" message**: the time-of-day greeting at 26px
...
**Save a restore point now.** **save one now**, in the modal's header, is a mono
...
it is something Addison is telling you about the record, not a control, and it has
...
Developer, and the panel is gone, not greyed). Exactly TWO guards, nothing else:
...
destructive command, and a DIFFERENT destructive command from the same tool then
...
did, then put it back. The confirm must say so, as a two-step inline confirm and
never a browser dialog: *"You've changed this file since Addison did. Reverting
will replace what's there now with the version from before Addison's first change."*
