...
2. ~~**`shell.readWorkspaceFile` has no size ceiling.**~~ **CLOSED 2026-08-08**, in its own
   PR as this entry asked. `read_workspace_path` did a bare `std::fs::read`, and `UNDO_SIZE_BOUND`
   ...
   **refuses** an oversized read in plain language: it never truncates, because a harness that
...
in vitest, and without fallbacks every theme test would silently assert nothing.
