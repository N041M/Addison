            ...
            <>
              ...
              <div className="relative mt-0.5">
                ...
                >
                  ...
                  <span className="text-[10px] text-faint">{trayOpen ? "▴" : "▾"}</span>
                </button>
              </div>
            </>
          )}
        ...
        {(editMode || inTray) && (
          <div className="flex shrink-0 items-center gap-1.5">
            <button
              ...
              className="text-[13px] leading-none text-muted hover:text-fern-deep"
            >
              {widget.pinned ? "⬤" : "◯"}
            </button>
            {editMode && (
              <button
                ...
                className="text-[13px] leading-none text-faint hover:text-danger"
              >
                ✕
              </button>
            )}
    ...
    <>
      ...
      {filling && (
        <div className="mt-2.5 rounded border border-line bg-paper p-2.5">
          {needsInput.map((v) => (
            <label key={v.name} className="mb-2 block text-[12px] font-medium text-ink-soft">
              {v.prompt}
              <input
                type="text"
                value={values[v.name] ?? ""}
                onChange={(e) => setValues((prev) => ({ ...prev, [v.name]: e.target.value }))}
                className="mt-1 w-full rounded border border-line bg-surface px-2 py-1.5 text-[13px] text-ink"
              />
            </label>
          ))}
          <div className="mt-1 flex gap-2">
            <button
              type="button"
              onClick={() => void run()}
              className="rounded-sm bg-fern px-3 py-1 text-[12px] font-semibold text-on-accent hover:bg-fern-deep"
            >
              Start
            </button>
            <button
              ...
              className="rounded-sm border border-line bg-surface px-3 py-1 text-[12px] font-medium text-ink-soft"
            >
              Cancel
            </button>
          </div>
      ...
      {outcome && (
        <p className={"mt-2 text-[11.5px] " + (outcome.ok ? "text-fern-deep" : "text-ink-soft")}>
          {outcome.detail}
        </p>
      )}
    </>
  ...
  return (
    <>
      <div className="flex items-baseline justify-between">
        ...
        <span className="font-mono text-[11.5px] text-ink-soft">{value}</span>
      </div>
    ...
    <div className="flex flex-col gap-[5px]">
      ...
      {rows.map((c) => (
        <div key={c.id} className="flex items-center gap-[7px]">
          ...
          />
          <span className="min-w-0 truncate text-[12px] text-ink">{c.label}</span>
          <span className="ml-auto shrink-0 font-mono text-[10px] text-faint">{c.detail}</span>
        </div>
      ))}
    </div>
  );
...
function LatencyBody({ title, stats }: { title: string; stats: Stats | null }) {
  ...
  return (
    <div className="flex flex-col gap-[5px]">
      ...
      {rows.length === 0 && <p className="text-[11.5px] text-faint">No calls yet.</p>}
      {rows.map((r) => (
        <div key={r.provider} className="flex items-center gap-[7px]">
          <span className="min-w-0 truncate text-[12px] text-ink">{r.provider}</span>
          <span className="ml-auto shrink-0 font-mono text-[10px] text-faint">{r.ms} ms</span>
        </div>
      ))}
    </div>
  );
...
function SmallCaps({ children, className = "" }: { children: ReactNode; className?: string }) {
  return (
    <span
      className={
        "text-[11px] font-semibold uppercase tracking-[0.06em] text-faint " + className
      }
    >
      {children}
    </span>
